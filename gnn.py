...
TEST_BATCH_SIZE = 32
GNN_HIDDEN_DIM = 256
GNN_KNN = 5  # number of nearest neighbours used to build the graph
...
class GraphConvLayer(nn.Module):
    ...
    Given node features H and a normalised adjacency matrix A_hat, computes:
        H' = ReLU(A_hat @ H @ W)
    where A_hat = D^{-1/2} A D^{-1/2} (symmetric normalisation).
    The adjacency matrix A is built dynamically per batch using cosine
    similarity between node feature vectors and a k-NN sparsification step.
    """
    ...
    def forward(self, h, adj):
        # h   : (N, in_features)
        # adj : (N, N) normalised adjacency (symmetric)
        out = torch.mm(adj, h)       # aggregate neighbours
...
def build_knn_graph(features, k):
    """
    Build a symmetric k-NN graph adjacency matrix from feature embeddings
    using cosine similarity.

    Args:
        features : (N, D) tensor of node feature vectors
        k        : number of nearest neighbours (excluding self)

    Returns:
        adj_hat  : (N, N) symmetric normalised adjacency matrix (on same device)
    """
    N = features.size(0)
    # cosine similarity matrix
    norm = F.normalize(features, p=2, dim=1)          # (N, D)
    sim = torch.mm(norm, norm.t())                     # (N, N)

    # keep only top-k neighbours (excluding self)
    sim.fill_diagonal_(float('-inf'))
    topk_vals, topk_idx = torch.topk(sim, k=min(k, N - 1), dim=1)
...
class CNN_GNN(nn.Module):
    """
    Combined CNN + GNN model for multilabel classification.

    The CNN backbone (ResNet) extracts a feature vector for each image in the
    batch.  Those vectors become nodes in a k-NN graph that is built on the
    fly.  Two GCN layers then refine the embeddings by propagating information
    between visually similar images before the final classifier head.
    """
    def __init__(self, block, num_blocks, num_classes, gnn_hidden=GNN_HIDDEN_DIM, k=GNN_KNN):
        ...
        # GNN layers
        self.gcn1 = GraphConvLayer(cnn_out_dim, gnn_hidden)
        self.gcn2 = GraphConvLayer(gnn_hidden, gnn_hidden)
    ...
    def forward(self, x):
        ...
        h = self.flatten(out)               # (N, 512)

        # --- Graph construction ---
        adj = build_knn_graph(h.detach(), self.k)   # (N, N)

        # --- GNN message passing ---
        h = self.gcn1(h, adj)               # (N, gnn_hidden)
        h = self.gcn2(h, adj)               # (N, gnn_hidden)
...
def UseModel(model, dataset_train, dataset_val, dataset_test):
    ...
    header = "date;time;res_blocks;classes;learning_rate;weight_decay;weight_parameter;Threshold;gnn_hidden;gnn_knn;epochs;early_stopping;train_transforms;test_transforms;precision;recall;f1_score\n"
...
if __name__ == '__main__':
    ...
    model = CNN_GNN(ResidualBlock, RES_BLOCKS, NUM_CLASSES).to(DEVICE)
