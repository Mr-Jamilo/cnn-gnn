...
class CNNGNNModel(nn.Module):
    def __init__(self, opt, cnn_blocks, extraction_layer, gnn_channels, num_classes, k, drop_path):
        ...
        self.gnn = GNNClassifier(opt=opt, in_channels=self.cnn.out_channels, in_spatial=self.cnn.out_spatial, num_classes=num_classes, channels=gnn_channels, k=k, drop_path=drop_path)
...
class EarlyStopping:
    ...
    def load_best_model(self, model):
        torch.save(self.best_model_state, "cnn-gnn-multilabel.pth")
...
if __name__ == "__main__":
    ...
    model = CNNGNNModel(opt=opt, cnn_blocks=opt.cnn_res_blocks_list, extraction_layer=opt.cnn_extraction_layer, gnn_channels=gnn_channels, num_classes=NUM_CLASSES, k=opt.k_neighbours, drop_path=opt.stochastic_path,).to(DEVICE)
